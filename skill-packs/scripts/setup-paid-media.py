...
PACK_FILE = (
    Path(__file__).resolve().parent.parent / "paid-media-pack.md"
)
...
class SkillPackSetup:
    ...
    def run_health_checks(self):
        ...
        for skill_name in self.configured_skills:
            ...
            if not health_cmd:
                print(f"  {meta['label']}: No automated health check (verify manually in Claude Code)")
                continue
...
Examples:
  ...
  python scripts/setup.py --non-interactive
    ...
    # Pack composition lives in paid-media-pack.md frontmatter; validate
    # the --skills filter against that list, not against SKILL_METADATA.
