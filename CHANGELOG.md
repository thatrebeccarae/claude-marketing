...
- **`klaviyo-analyst` is now MCP-first (v1.1.0).** SKILL.md install steps lead with the official Klaviyo MCP server (`https://mcp.klaviyo.com/mcp`, OAuth + dynamic client registration, `?read-only=true` for audit-only sessions) and the `uvx klaviyo-mcp-server@latest` local alternative. The Python scripts are demoted to a "Fallback: Local CLI" section for CI/cron use cases. A Cowork callout in the 4-Phase Audit workflow notes that the audit is built for unattended execution. Troubleshooting and Security Notes sections now split between MCP-path and script-path guidance.
- **`klaviyo-developer` MCP awareness (v1.1.0).** New "MCP vs. SDK: When to Use Which" section near the top of the skill explains when to reach for the MCP (schema exploration, event-firing debugging, ad-hoc inventory) and when to stay with the SDK (production code, bulk imports, webhooks, CI). The skill remains SDK-first by design.
- **MCP Server Reference appendices** in both `klaviyo-analyst/REFERENCE.md` and `klaviyo-developer/REFERENCE.md`. Documents the full Klaviyo MCP tool inventory (40+ tools across Accounts, Campaigns, Catalogs, Events & Metrics, Flows, Groups, Images, Profiles, Reporting, Templates, Translations), read/write status per tool, OAuth setup, local install via `uvx`, read-only URL flag, required Klaviyo user role (Owner/Admin/Manager), and API revision pin (`2026-04-15`). The developer reference also includes integration debugging recipes mapping common problems to MCP tool sequences.
- **Root README Klaviyo MCP setup expanded** to document both the remote OAuth path (now Option A, recommended) and the local `uvx` path (Option B). The Klaviyo skill catalog rows note MCP-first status for the analyst skill and MCP-aware status for the developer skill.
